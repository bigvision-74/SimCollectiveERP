...
const SessionSetup: React.FC<SessionSetupProps> = ({ wardData, onCancel }) => {
  ...
  // Format current date for min attribute in datetime-local input
  ...
  const handleDurationChange = (value: string | number) => {
    if (value === "unlimited") {
      ...
      const now = new Date();
      setLocalDateTimeString(getCurrentDateTimeForInput());
  ...
  const handleAutoAssign = () => {
    ...
    patients.forEach((patient, index) => {
      ...
      switch (zoneIndex) {
        case 0: zoneId = "zone1"; break;
        case 1: zoneId = "zone2"; break;
        case 2: zoneId = "zone3"; break;
        case 3: zoneId = "zone4"; break;
        default: zoneId = "zone1";
      }

      if (newAssignments[zoneId].length < 3) {
        newAssignments[zoneId].push(patient);
      } else {
        const zones: Array<keyof typeof newAssignments> = ["zone1", "zone2", "zone3", "zone4"];
        for (let i = 0; i < 4; i++) {
  ...
  // --- FIX: Merge returning patients with existing unassigned list ---
  ...
  const handleDrop = (e: React.DragEvent, targetZone: string) => {
    ...
    if (targetZone !== "unassigned" && assignments[targetZone as AssignmentKey].length >= 3) {
      alert("This zone is full (Max 3 patients).");
      return;
    }
  ...
  const handleStartSession = async () => {
    ...
    if (selectedDuration === "unlimited") {
      if (!selectedDateTime || !calculatedDuration || calculatedDuration <= 0) {
        alert("Please select a valid future end date and time");
        return;
      }
    ...
    if (wardData.faculty?.id) formattedAssignments["faculty"] = [wardData.faculty.id];
    if (wardData.observer?.id) formattedAssignments["Observer"] = [wardData.observer.id];
    ...
    zonesConfig.forEach((zone) => {
      ...
      formattedAssignments[zone.id] = { userId: userId, patientIds: patientIds };
    });
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 mb-5 relative">
        <div className="w-full lg:w-auto flex justify-between items-center">
          <div>
            ...
            <div className="text-xs text-slate-500 mt-1">{t("Dragpatients")}</div>
          </div>
        </div>
        ...
        <div className="w-full lg:w-auto grid grid-cols-1 sm:grid-cols-3 lg:flex items-center px-4 py-3 lg:px-6 lg:py-2 bg-white dark:bg-darkmode-600 rounded-xl border border-slate-200 dark:border-darkmode-400 shadow-sm gap-4 lg:gap-6">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">{t("Ward")}</div>
              <div className="text-xs font-bold text-slate-700 dark:text-slate-200 truncate">{wardData.name}</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="relative shrink-0">
              <img
                src={wardData.faculty?.user_thumbnail || "https://via.placeholder.com/150"}
                alt="Faculty"
                className="w-8 h-8 rounded-full border-2 border-white shadow-sm object-cover"
              />
            </div>
            <div className="min-w-0">
              <div className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">{t("faculty")}</div>
              <div className="text-xs font-bold text-slate-700 dark:text-slate-200 truncate">
                {wardData.faculty ? `${wardData.faculty.fname} ${wardData.faculty.lname}` : "N/A"}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <img
              src={wardData.observer?.user_thumbnail || "https://via.placeholder.com/150"}
              alt="Observer"
              className="w-8 h-8 rounded-full border-2 border-white shadow-sm object-cover grayscale opacity-80 shrink-0"
            />
            <div className="min-w-0">
              <div className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">{t("Observer")}</div>
              <div className="text-xs font-bold text-slate-700 dark:text-slate-200 truncate">
                {wardData.observer ? `${wardData.observer.fname} ${wardData.observer.lname}` : "N/A"}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap w-full lg:w-auto items-center gap-3 justify-end">
          <div className="relative w-full sm:w-auto">
            <select
              ...
              onChange={(e) => handleDurationChange(e.target.value === "unlimited" ? "unlimited" : Number(e.target.value))}
            ...
            >
              <option value="" disabled className="text-slate-400 dark:text-slate-500">Duration...</option>
              {durationOptions.map((opt) => (
                <option key={opt.label} value={opt.value} className="py-2">{opt.label}</option>
              ))}
            </select>
          </div>
          ...
          {showCalendar && (
            <div className="absolute z-50 mt-2 top-full right-0 bg-white dark:bg-darkmode-700 rounded-lg shadow-xl border border-slate-200 dark:border-darkmode-600 p-4 w-full sm:w-[320px]">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-bold text-slate-700 dark:text-slate-200">Select End Date & Time</h3>
                <Button
              ...
              <div className="mb-3">
                ...
                <input
                  ...
                  className="w-full border border-slate-300 dark:border-darkmode-400 rounded px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-transparent"
                />
              </div>
              ...
              {selectedDateTime && (
                <div className="p-3 bg-slate-50 dark:bg-darkmode-800 rounded-md mb-3">
                   <div className="text-sm text-slate-600 dark:text-slate-300">
                     <span className="font-bold">{formatDateTime(selectedDateTime)}</span>
                     {calculatedDuration !== null && calculatedDuration > 0 && (
                        <div className="text-xs text-slate-500 mt-1">Duration: {calculatedDuration} mins</div>
                     )}
                   </div>
                </div>
              )}
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button
                  variant="primary"
                  size="sm"
                  disabled={!selectedDateTime || (calculatedDuration !== null && calculatedDuration <= 0)}
                  onClick={() => {
                    if (selectedDateTime && calculatedDuration && calculatedDuration > 0) setShowCalendar(false);
                  }}
                >
                  Confirm
                </Button>
          ...
          <div className="flex gap-2 w-full sm:w-auto justify-end">
            ...
            <div className="relative group flex-1 sm:flex-none">
              <Button
                variant="primary"
                disabled={!canStart || (selectedDuration === "unlimited" && !selectedDateTime)}
                onClick={handleStartSession}
                className={clsx(
                  "shadow-md w-full justify-center transition-all duration-200",
                  !canStart || (selectedDuration === "unlimited" && !selectedDateTime)
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden p-3 space-y-3">
            ...
            {assignments.unassigned.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-slate-400 min-h-[100px]">
                <Lucide icon="CheckCircle" className="w-10 h-10 mb-2 opacity-50" />
              ...
              return (
                ...
                >
                  ...
                  >
                    <div className="w-10 h-10 lg:w-12 lg:h-12 flex-none image-fit mr-3 lg:mr-4 border-2 border-white rounded-full shadow-sm">
                      <img
                        ...
                        src={assignedUser?.user_thumbnail || "https://via.placeholder.com/150"}
                      />
                    </div>
                    <div className="flex justify-between items-center w-full min-w-0">
                      <div className="flex-1 min-w-0 pr-2">
                        <div className="font-bold text-slate-800 text-sm lg:text-base truncate">
                          {assignedUser ? `${assignedUser.fname} ${assignedUser.lname}` : "Unassigned"}
                        </div>
                        <div className="text-xs text-slate-500 truncate">{t("Students")}</div>
                      </div>
                      {hasPatients && (
                        <Button
                          variant="outline-danger"
                  ...
                  >
                    ...
                    {[...Array(3 - assignments[zone.id].length)].map((_, i) => (
                      <div
                        key={i}
                        className={clsx(
                          "h-12 lg:h-14 border-dashed rounded-lg flex items-center justify-center transition-all duration-300",
                          isHovered
                            ? clsx(zone.borderColor, "bg-white border-[1px]")
                            : "border-slate-200 bg-slate-50 border-[2px]"
                        )}
                      >
                        <Lucide
                          icon="Bed"
                          className={clsx("w-4 h-4 mr-2", isHovered ? zone.textColor : "text-slate-400")}
                        />
                        <span className={clsx("text-xs font-medium transition-colors", isHovered ? zone.textColor : "text-slate-400")}>
