...
function Main() {
  ...
  const handleAccountClick = async (emailOrUsername: string) => {
    setLoading(true);
    setShowAlert(null);

    const firebaseUser = await reauthenticateWithToken(emailOrUsername);
    ...
    if (firebaseUser && firebaseUser.email) {
      try {
        ...
        setFormData({ username: emailOrUsername, password: "" });
        setShowAccountChooser(false);
      }
    } else {
      setFormData({ username: emailOrUsername, password: "" });
  ...
  useEffect(() => {
    const savedUser = getCookie("email") || getCookie("username");
    if (savedUser) {
      setFormData((prev) => ({ ...prev, username: savedUser }));
    }
  }, []);

  const trackUserLocation = async (userId: any) => {
    ...
    FormDataOnlineUser.append("longitude", longitudeData || "null");
    FormDataOnlineUser.append("city", cityData || "null");
    FormDataOnlineUser.append("userid", userId);

    await addOnlineUserAction(FormDataOnlineUser);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (errMsg === t("Usernotfound")) message = t("loginError1");
      else if (errMsg === t("Useraccounthasbeendeleted"))
        message = t("loginError2");
      else if (errMsg === t("Organizationhasbeendeleted"))
        message = t("loginError3");
      else if (errMsg === t("Invalidemailorpassword"))
        message = t("loginError4");
                ...
                <div>
                  ...
                  <FormInput
                    ...
                    className={clsx(
                      "w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition",
                      {
                        "border-gray-300": !formErrors.username,
