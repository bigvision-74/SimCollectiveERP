...
const checkAndEndExpiredSessions = async () => {
  ...
  try {
    ...
    for (const session of activeSessions) {
      ...
      const expirationTime = new Date(startTime.getTime() + durationInMinutes * 60000);
      ...
      if (now >= expirationTime) {
        ...
        const createdByUser = await knex("users").where({ id: session.createdBy }).first();
        if (createdByUser && createdByUser.organisation_id) {
          io.to(createdByUser.organisation_id.toString()).emit("session:ended", {
