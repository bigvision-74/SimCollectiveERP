...
exports.addPermissions = async (req, res) => {
  ...
  try {
    ...
    const existingUserPermission = await knex("user_permissions")
      .where({ user_id: id })
      .first();
    ...
    if (existingUserPermission) {
      await knex("user_permissions")
        .where({ user_id: id })
        .update({
          permission_name: JSON.stringify(permissionsArray),
    ...
    console.error("Error updating or adding permissions:", error);
    return res
      .status(500)
      .json({ message: "Failed to add or update permissions", error });
  }
};
...
exports.getPermissions = async (req, res) => {
  ...
  try {
    ...
    if (!userPermissions) {
      return res
        .status(404)
        .json({ message: "User not found or no permissions set" });
    }

    return res.status(200).json({
      user_id: userPermissions.user_id,
      permissions: JSON.parse(userPermissions.permission_name),
    });
  } catch (error) {
    console.error("Error retrieving permissions:", error);
    return res
      .status(500)
      .json({ message: "Failed to retrieve permissions", error });
  }
};
